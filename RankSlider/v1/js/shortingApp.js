...
$("document").ready(function () {
  ...
  var toolTextArr=toolText.split("|");
...
$(".rank-slider").append(' <div class="tool-information">'+toolinfo+'</div> <div class="mainWaper">\
<div class="UndoButton">\
  <span class="undo-btn"></span\
  ><span class="undo-text">'+toolTextArr[0]+'</span>\
</div>\
<div class="listingNumber">\
  <ul></ul>\
</div>\
<div class="mealclass">\
  <div class="hintText1">'+toolTextArr[1]+'</div>\
  <div class="hintText2">'+toolTextArr[2]+'</div>\
  <ul id="meallist"></ul>\
  <div class="clearAll"></div>\
</div>\
<div class="clearAll"></div>\
</div>\
<div class="overlapAll"></div>\
<div class="overlapblock">\
...
<div class="popup-header">\
  <div class="popHeading">'+toolTextArr[3]+'</div>\
...
<div class="contant-block">\
  ...
  <div class="footer-block">\
    <div class="btn_cancel">'+toolTextArr[4]+'</div>\
    <div class="btn_submit">'+toolTextArr[5]+'</div>\
  </div>\
</div>\
</div>')
  ...
  function resetSideNumbers() {
    ...
    $("#meallist li").each(function (index) {
      ...
      if ($(this).attr("class") == "imgBlock ui-state-default") {
        $(".listingNumber ul").append(
          '<li><div class="numBlock1"><span><span class="Showlist">' +
            (index + 1) +
            "</span></span></div></li>"
        );
      } else {
        $(".listingNumber ul").append(
          '<li><div class="numBlock2"><span><span class="Showlist">' +
            (index + 1) +
            "</span></span></div></li>"
        );
      }
  ...
  function shuffleArray(d, m) {
    ...
    return {
      ...
      indxArr: m,
      undoObj: {
        undoStatement: unduStatementStr_,
        undoStatID: undoPreCode_,
      },
    };
  }
  ...
  if (backFunctionality == true) {
    ...
    newArr.forEach(function (el, index) {
      if (
        imgArry_[el].split(".")[imgArry_[el].split(".").length - 1] == "jpg" ||
        imgArry_[el].split(".")[imgArry_[el].split(".").length - 1] == "jpeg" ||
        imgArry_[el].split(".")[imgArry_[el].split(".").length - 1] == "png" ||
        imgArry_[el].split(".")[imgArry_[el].split(".").length - 1] == "gif"
      ) {
        $("#meallist").append(
          '<li id="' +
            precodeStr_[el] +
            '" class="imgBlock ui-state-default"><div class="overimgBlock"><img src="' +
            imgArry_[el] +
            '"></div><div class="zoomIconImg zoomIcon cancelSort" data-info="' +
            imgArry_[el] +
            '">X</div><div class="closeList cancelSort" data-remove="' +
            precodeStrArr[i] +
            '">X</div></li>'
        );
        ...
        $("#meallist").append(
          '<li id="' +
            precodeStr_[el] +
            '" class="txtBlock ui-state-default"><div class="overtxtBlock">' +
            imgArry_[el] +
            '</div><div class="zoomIconText zoomIcon cancelSort" data-info="' +
            imgArry_[el] +
            '">X</div><div class="closeList cancelSort" data-remove="' +
            precodeStr_[el] +
            '"> X</div></li>'
    ...
    for (var i = 0; i < statementStr.length; i++) {
      if (
        randomObj.imArr[i]
          .split("/")
          [randomObj.imArr[i].split("/").length - 1].split(".")[1] == "jpg" ||
        randomObj.imArr[i]
          .split("/")
          [randomObj.imArr[i].split("/").length - 1].split(".")[1] == "png" ||
        randomObj.imArr[i]
          .split("/")
          [randomObj.imArr[i].split("/").length - 1].split(".")[1] == "jpeg" ||
        randomObj.imArr[i]
          .split("/")
          [randomObj.imArr[i].split("/").length - 1].split(".")[1] == "gif"
      ) {
        $("#meallist").append(
          '<li id="' +
            precodeStrArr[i] +
            ...
            '"></div><div class="zoomIconImg zoomIcon cancelSort" data-info="' +
            randomObj.imArr[i] +
            '"> </div><div class="closeList cancelSort" data-remove="' +
            precodeStrArr[i] +
            '"> X</div></li>'
        );
        $(".listingNumber ul").append(
  ...
  randomObj.undoObj.undoStatement.forEach(function (el, index) {
    if (
      randomObj.undoObj.undoStatement[index]
        .split("/")
        [randomObj.undoObj.undoStatement[index].split("/").length - 1].split(
          "."
        )[1] == "jpg" ||
      randomObj.undoObj.undoStatement[index]
        .split("/")
      ...
      $(".contant-body ul").append(
        '<div><input type="checkbox" id="' +
          randomObj.undoObj.undoStatID[index] +
          '_"><label for="' +
          randomObj.undoObj.undoStatID[index] +
          '_"><li id="' +
          randomObj.undoObj.undoStatID[index] +
          ...
          '"></div><div class="zoomIconText zoomIcon cancelSort" data-info="' +
          el +
          '" style="display: block;"> </div><div class="closeList cancelSort" data-remove="' +
          randomObj.undoObj.undoStatID[index] +
          '" style="display: block;">X</div></li></label></div>'
      );
    } else {
      $(".contant-body ul").append(
        '<div><input type="checkbox" id="' +
  ...
  $(function () {
    ...
    $(".zoomIconImg").click(function () {
      ...
      $(".largeimage").html(
        '<div class="popimgcontant"><img src="' + currentLargeImg + '"></div>'
      );
    });
    $(".zoomIconText").click(function () {
      ...
      $(".overlapblock, .closeIcon, .overlapAll").show();
      $(".largeimage").html(
        '<div class="popcontant">' + currentLargeImg + "</div>"
      );
    });
    ...
    $(".closeList").click(function (index) {
      if ($("#meallist li").length != 1) {
        ...
        $(element).wrap(
          '<div class="undo-wrapper"><label for="' + elId + '_"></label></div>'
        );

        $(element)
          .parent()
          .parent()
          .prepend('<input type="checkbox" id="' + elId + '_"/>');

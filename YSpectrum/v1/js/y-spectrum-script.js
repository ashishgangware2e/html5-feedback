$(document).ready(function() {
    ...
    $(".toolWapper").html('<div class="target-row stmt-row clearfix"><div class="setMiddle"><div class="blockElments" id="statmentText"></div></div></div><div class="blockElments margin_bottom" id="countdownList"> </div><div class="target-row drp-zone clearfix"><div class="bottomRightNegative">' + legentText[1] + '</div><div class="toTarget y-axis"><div class="midset clearfix"><div class="clickableArea"><div class="bdr-clr"></div></div></div></div><div class="clearAll"></div><div class="BottomleftPosetive">' + legentText[0] + '</div></div>');

    $("#countdownList").html('<div class="countBox">' + topCountStrArr[0] + ' <span class="count-number">' + (0)  + '</span></div>');

    for (var i = 0; i < statmentStrArr.length; i++) {
        $("#statmentText").append('<div class="borderBox" data-index="' + i + '" id="movetext_' + StatmentCodeArr[i] + '" style="z-index:' + (zindex) + '; top: '+border_top+'px; left:'+border_left+'px " >' + statmentStrArr[i] + '</div>');
        zindex = zindex - 1;
    ...
    $('.clickableArea, .bdr-clr').bind('click', function(e) {
        ...
        if (!$(".borderBox").length == 0 && ends == true) {
            ...
            console.log(flagParaCount - 1);

            if (flagParaCount >= (statmentStrArr.length) + 1) {
                $("#countdownList").remove();
            }
            $("#countdownList").html('<div class="countBox">' + topCountStrArr[0] + ' <span class="count-number">' + (flagParaCount - 1) + '</span><div>');

...
$(document).ready(function(){
			...
			drop:function(e,ui){
				...
				if(ranges>50){
					...
					if($(ui.draggable[0]).hasClass("stmnt")){
						...
						var nlft = e.pageX-$(".lableTxt").offset().left;
						var ntop = e.pageY-$(".lableTxt").offset().top; 
						...
						if(cnt>=totalCount){
						 ...
						 $(".lableTxt").find(".palced-item").html(Item+' <span class="counter">'+(cnt-1)+'</span> '+of+' '+totalCount);
						 if(totalCount+1 ===cnt){
							 $(".palced-item").css({display:"none"});
							 $(".palced-item-mssg").css({display:"block"});
						 }
						...
						if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
						   ...
						   $(this).append("<div class='dots ' style='left:"+(lftPrcnt+2)+"%; top:"+(topPrcnt)+"%;'><div class='dragable-text'>"+drgArr[cnt-2]+"</div></div>");
						}else{
							$(this).append("<div class='dots' style='left:"+(lftPrcnt+2)+"%; top:"+(topPrcnt)+"%;'><div class='dragable-text '>"+drgArr[cnt-2]+"</div></div>");
						}
						dragDots();
						...
						console.log("ser;")			
						opt[randomIndx[cnt-1]]=Math.round(ranges*2);
						outputValues(opt);	
	...
	dragItem();
	 ...
	 function countUpdate(count,totalCount){
		 ...
		 $(".leftdiv").html('<div class="lableTxt"><span class="palced-item">'+Item+' <span class="counter">'+(count-1)+'</span> '+of+' '+totalCount+'</span><span class="palced-item-mssg">'+[AllitemPlaced]+'</span></div>'+dvs+"<div class='circleEle'>"+circle+"</div>");
		 centrediv();
	}

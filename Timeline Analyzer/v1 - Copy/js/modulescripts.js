$(window).load(function(){
	...
	for(var i=0; i<dragStatementArr.length; i++){
		$(".dragContaner ul").append('<li><div id="draggable'+precodeArr[i]+'" scroll-issue="1" class="draggablelist pset"><span class="item">'+dragStatementArr[i]+'<div class="colPic"></div></span></div></li>');
	}
...
function toolbinding(){
    ...
    $("li .draggablelist").draggable({
		...
		drag: function(e, ui){
			...
			$(".speech").unbind("click touchstart").bind("click touchstart",speechEvent);
    ...
    $("#droppable").droppable({
		drop: function(event, ui) {
			...
			if(dropArea){
				...
				$("textarea").unbind("click touchstart").bind("click touchstart",function(){$(this).focus();});
				getUserComments();
				//active
			} else {
			...
			$("span.span").unbind("click touchstart").bind("click touchstart",function(e){
				e.stopPropagation();
				$(".speech").addClass("small");
				$("textarea").blur();
			});
...
// END HIGH CHART SCRIPT	
	...
	window.addEventListener("resize", function(){
		...
		// if(outputValues.length != 0){ //commented in analyzer

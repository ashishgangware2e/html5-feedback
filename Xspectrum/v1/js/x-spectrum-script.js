$(document).ready(function() {
    ...
    var shadow = StatmentCodeArr.length;
    var flagCount = 0;
    flagParaCount = 1;
    var outputarr = [];
    var backOutputarr = [];

    $(".toolWapper").html('<div class="target-row"><div class="setMiddle"><div class="blockElments" id="statmentText"></div></div><div class="blockElments margin_bottom" id="countdownList"> </div></div><div class="target-row"> <div class="toTarget"><div class="midset"><div class="clickableArea"><div class="bdr-clr"></div></div></div><div class="bottomRightNegative">' + legentText[1] + '</div><div class="BottomleftPosetive">' + legentText[0] + '</div></div><div class="target-overlay"></div></div>');

    $("#countdownList").html('<div class="countBox"><span class="count-text">'+topCountStrArr[0]+"</span><span class='count-number'>0</span>" +'</div>');

    for (var i = 0; i < statmentStrArr.length; i++) {
        $("#statmentText").append('<div class="borderBox" data-index="' + i + '" id="movetext_' + StatmentCodeArr[i] + '" style="z-index:' + (zindex) + '; right:'+shadow+'px; top:'+shadow+'px"><div class="v-algn">' + statmentStrArr[i] + '</div></div>');
        zindex = zindex - 1;
        shadow = shadow - 5;
    }
    ...
    $('.clickableArea, .bdr-clr').bind('click', function (e) {
        ...
        if (!$(".borderBox").length == 0 && ends==true) {
            ...
            $("#countdownList").html('<div class="countBox"><span class="count-text">'+topCountStrArr[0]+'</span> <span class="count-number">' + flagParaCount + '</span></div>');
